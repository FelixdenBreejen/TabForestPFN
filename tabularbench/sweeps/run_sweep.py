...
from tabularbench.sweeps.run_status import (all_runs_almost_finished,
                                            all_runs_finished,
                                            no_runs_finished)
...
def run_sweep(cfg: ConfigBenchmarkSweep):
    ...
    runs_per_dataset = get_runs_per_dataset(cfg)
    ...
    while not all_runs_finished(cfg, run_results_dict, runs_per_dataset):
        ...
        if all_runs_almost_finished(cfg, run_results_dict, runs_per_dataset, runs_busy_dict):
            ...
            process_results(cfg, run_results_dict)
            continue
...
def get_runs_per_dataset(cfg: ConfigBenchmarkSweep) -> int:
...
def process_results(cfg: ConfigBenchmarkSweep, run_results_dict: dict) -> None:

    if no_runs_finished(run_results_dict):
        return
    
    results_sweep = ResultsSweep.from_run_results_dict(run_results_dict)
    results_sweep.save(cfg.output_dir / RESULTS_FILE_NAME)
...
def draw_dataset_id(cfg: ConfigBenchmarkSweep, run_results_dict: dict[int, list[ResultsRun]], runs_per_dataset: int, runs_busy_dict: dict[int, int]) -> int:
